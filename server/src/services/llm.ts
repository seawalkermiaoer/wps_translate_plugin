 ...
 * @returns {string}
 */
function buildStrictSystemPrompt(missingTags: string[]): string {
  return (
    SYSTEM_PROMPT +
    `
 ...
 */
export function validateTagsInTranslation(sourceText: string, translatedText: string): boolean {
 ...
 */
export function getMissingTags(sourceText: string, translatedText: string): string[] {
  const tagIds = extractTagIds(sourceText);
